...
function updateColors(type, color) {
    console.log('Updating', type, 'to', color.name);
    if (type === 'primary') {
...
// Aplicar tema noir na inicialização
onMounted(() => {
    console.log('Configurando tema noir...');

    // Garantir que o primary seja noir
    layoutConfig.primary = 'noir';

    // Encontrar a cor noir
    const noirColor = primaryColors.value.find((c) => c.name === 'noir');
    if (noirColor) {
        console.log('Aplicando tema noir:', noirColor);

        // Aplicar o tema completamente
        updatePreset(getPresetExt());

        // Aplicar também via função applyTheme para garantir
        applyTheme('primary', noirColor);
    }

    // Usar setTimeout para garantir que seja aplicado após o DOM estar pronto
    setTimeout(() => {
        if (layoutConfig.primary === 'noir') {
            updatePreset(getPresetExt());
        }
    }, 100);
});

// Watcher para garantir que sempre seja noir
watch(
...
<template>
    <div
        class="config-panel hidden absolute top-[3.25rem] right-0 w-64 p-4 bg-surface-0 dark:bg-surface-900 border border-surface origin-top shadow-[0px_3px_5px_rgba(0,0,0,0.02),0px_0px_2px_rgba(0,0,0,0.05),0px_1px_4px_rgba(0,0,0,0.08)]"
    >
